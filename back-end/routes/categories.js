...
router.get('/', asyncHandler(async function (req, res, next) {
  ...
  #swagger.responses[500] = {description: 'This response is returned when an **internal server error** occurs while the API is trying to get all products', schema: {$ref: '#/definitions/categoriesGetInternalError'}}
  */
...
router.post('/', isAuth, isAdmin, asyncHandler(async function (req, res, next) {
  ...
  #swagger.responses[500] = {description: 'This response is returned when an **internal server error** occurs while the API is trying to create a new category', schema: {$ref: '#/definitions/categoriesPostInternalError'}}
...
router.put('/:id', isAuth, isAdmin, asyncHandler(async function (req, res, next) {
  ...
  #swagger.responses[500] = {description: 'This response is returned when an **internal server error** occurs while the API is trying to update a category', schema: {$ref: '#/definitions/categoriesUpdateInternalError'}}
  #swagger.security = [{adminAuth: []}]
  */
  const { name } = req.body;
  const expectedType = { name: 'string' };
...
router.delete('/:id', isAuth, isAdmin, asyncHandler(async function (req, res, next) {
  ...
  #swagger.responses[500] = {description: 'This response is returned when an **internal server error** occurs while the API is trying to delete a category', schema: {$ref: '#/definitions/categoriesDeleteInternalError'}}
  #swagger.security = [{adminAuth: []}]
  */
